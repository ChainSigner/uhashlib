...
STATIC mp_obj_t hashlib_keccak256_update(mp_obj_t self_in, mp_obj_t arg);

STATIC mp_obj_t hashlib_keccak256_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args) {
    mp_arg_check_num(n_args, n_kw, 0, 1, false);
    mp_obj_hash_t *o = m_new_obj_var(mp_obj_hash_t, char, sizeof(SHA3_CTX));
    o->base.type = type;
    keccak_256_Init((SHA3_CTX*)o->state);
    if (n_args == 1) {
        hashlib_keccak256_update(MP_OBJ_FROM_PTR(o), args[0]);
    }
    return MP_OBJ_FROM_PTR(o);
}

STATIC mp_obj_t hashlib_keccak256_copy(mp_obj_t self_in) {
    mp_obj_hash_t *o = m_new_obj_var(mp_obj_hash_t, char, sizeof(SHA3_CTX));
    mp_obj_hash_t *self = MP_OBJ_TO_PTR(self_in);
    o->base.type = self->base.type;
    memcpy(o->state, self->state, sizeof(SHA3_CTX));
    return MP_OBJ_FROM_PTR(o);
}
...
STATIC mp_obj_t hashlib_keccak256_update(mp_obj_t self_in, mp_obj_t arg) {
    ...
    mp_buffer_info_t bufinfo;
    mp_get_buffer_raise(arg, &bufinfo, MP_BUFFER_READ);
    keccak_Update((SHA3_CTX*)self->state, bufinfo.buf, bufinfo.len);
    return mp_const_none;
}

STATIC mp_obj_t hashlib_keccak256_digest(mp_obj_t self_in) {
    mp_obj_hash_t *self = MP_OBJ_TO_PTR(self_in);
    vstr_t vstr;
    vstr_init_len(&vstr, SHA3_256_DIGEST_LENGTH);
    keccak_Final((SHA3_CTX*)self->state, (byte*)vstr.buf);
    return mp_obj_new_str_from_vstr(&mp_type_bytes, &vstr);
}

STATIC MP_DEFINE_CONST_FUN_OBJ_2(hashlib_keccak256_update_obj, hashlib_keccak256_update);
STATIC MP_DEFINE_CONST_FUN_OBJ_1(hashlib_keccak256_digest_obj, hashlib_keccak256_digest);
STATIC MP_DEFINE_CONST_FUN_OBJ_1(hashlib_keccak256_copy_obj, hashlib_keccak256_copy);

STATIC const mp_rom_map_elem_t hashlib_keccak256_locals_dict_table[] = {
    { MP_ROM_QSTR(MP_QSTR_update), MP_ROM_PTR(&hashlib_keccak256_update_obj) },
    { MP_ROM_QSTR(MP_QSTR_digest), MP_ROM_PTR(&hashlib_keccak256_digest_obj) },
    ...
    { MP_ROM_QSTR(MP_QSTR_copy), MP_ROM_PTR(&hashlib_keccak256_copy_obj) },
};

STATIC MP_DEFINE_CONST_DICT(hashlib_keccak256_locals_dict, hashlib_keccak256_locals_dict_table);

STATIC const mp_obj_type_t hashlib_keccak256_type = {
    { &mp_type_type },
    .name = MP_QSTR_keccak256,
    .make_new = hashlib_keccak256_make_new,
    .locals_dict = (void*)&hashlib_keccak256_locals_dict,
};
...
STATIC mp_obj_t hashlib_new(size_t n_args, const mp_obj_t *args) {
    ...
    if(strcmp(typebuf.buf, "keccak256") == 0){
        return hashlib_keccak256_make_new(&hashlib_keccak256_type, n_args-1, 0, args+1);
    }
    if(strcmp(typebuf.buf, "sha512") == 0){
...
STATIC const mp_rom_map_elem_t hashlib_module_globals_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_keccak256), MP_ROM_PTR(&hashlib_keccak256_type) },
